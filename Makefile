...
etc/restic/gcs_env.sh:
	install -m 0600 etc/restic/gcs_env.sh.template etc/restic/gcs_env.sh

etc/restic/gcs_pw.txt:
	install -m 0600 etc/restic/gcs_pw.txt.template etc/restic/gcs_pw.txt

# target: install-conf - Install restic configuration files.
# will create these files locally only if they don't already exist
install-conf: | etc/restic/gcs_env.sh etc/restic/gcs_pw.txt
	install -d $(DEST_CONF)
